...
if (isset($_POST['submit']) && isset($_FILES['my_image'])) {
    ...
    if ($error === 0) {
        if ($img_size > 125000) {
            ...
            header("Location: ../views/admin_site.php?error=$em");
        } else {
            ...
            if (in_array($img_ex_lc, $allowed_exs)) {
                ...
                $img_upload_path = '../images/movies/' . $new_img_name;
                move_uploaded_file($tmp_name, $img_upload_path);

                // Insert into Database ----> $sql = "INSERT INTO images(image_url) VALUES('$new_img_name')";
                $sql = "UPDATE movies SET POSTER = '$new_img_name' WHERE MOVIE_ID = 1";

                mysqli_query($link, $sql);
                header("Location: ../views/admin_site.php");
            } else {
